...
function Header(){const [open,setOpen]=React.useState(false);const countdown=useElectionCountdown();return <>{siteConfig.electionMode?<div className="electionBar"><a href="#vision"><strong>IIA Telangana Elections · {siteConfig.electionDates}</strong><em>{countdown.value} {countdown.label}</em></a></div>:null}<header className={siteConfig.electionMode?'header':'header noElection'}><a className="brand" href="#top" aria-label="Dr. Aditya Singaraju, home"><span>AS</span><b>Dr. Aditya<br/>Singaraju</b></a><button className="menu" onClick={()=>setOpen(v=>!v)} aria-expanded={open} aria-label="Toggle menu">{open?<X/>:<Menu/>}</button><nav className={open?'open':''} aria-label="Primary navigation">{nav.map(([label,id])=><a key={id} href={'#'+id} onClick={()=>setOpen(false)}>{label}</a>)}<a className="navCta" href={siteConfig.electionMode?'#vision':'#research'}>{siteConfig.electionMode?'My Vision for IIA Telangana':'Explore My Work'} <ArrowRight/></a></nav></header></>}
...
function App(){return <><Header/><main id="top">
  ...
  <section id="research" className="professional"><SectionTitle number="05" title="Practice & research"/><div className="professionalIndex">{[
  ...
  ].map((item,i)=><article key={item[0]}><span>0{i+1}</span><div><h3>{item[0]}</h3><strong>{item[1]}</strong><p>{item[2]}</p></div></article>)}</div><div className="researchRail" aria-label="Research interests">{research.map(x=><span key={x}>{x}</span>)}</div></section>
