...
const nav = [['About','about'],['Journey','journey'],['IIA','iia'],['Vision','vision'],['Research & Practice','research']]
...
function Header(){const [open,setOpen]=React.useState(false);const countdown=useElectionCountdown();return <><div className="electionBar"><a href="#vision"><strong>IIA Telangana Chapter Elections</strong><span>{siteConfig.electionDates}</span><em>{countdown.value} {countdown.label}</em><b>Support My Candidature <ArrowRight/></b></a></div><header className="header"><a className="brand" href="#top" aria-label="Dr. Aditya Singaraju, home"><span>AS</span><b>Dr. Aditya<br/>Singaraju</b></a><button className="menu" onClick={()=>setOpen(v=>!v)} aria-expanded={open} aria-label="Toggle menu">{open?<X/>:<Menu/>}</button><nav className={open?'open':''} aria-label="Primary navigation">{nav.map(([label,id])=><a key={id} href={'#'+id} onClick={()=>setOpen(false)}>{label}</a>)}<a className="navCta" href={siteConfig.electionMode?'#vision':'#research'}>{siteConfig.electionMode?'Support My Candidature':'Explore My Work'} <ArrowRight/></a></nav></header></>}
...
function App(){const countdown=useElectionCountdown();return <><Header/><main id="top">
  <section className="hero ruled" aria-labelledby="hero-title"><div className="heroCopy"><h1 id="hero-title">Dr. Aditya<br/>Singaraju</h1><p className="roles">{profile.roles}</p><blockquote>“{profile.statement}”</blockquote>{siteConfig.electionMode?<div className="candidature"><span>Candidate for Chairperson</span><strong>Indian Institute of Architects — Telangana Chapter</strong><em>{siteConfig.electionDates}</em><div className="heroCountdown"><b>{countdown.value}</b><span>{countdown.label}</span><i>Election<br/>19–21 Aug</i></div></div>:<p className="intro">{profile.bio}</p>}<div className="actions"><a className="primary" href="#vision">Explore My Vision <ArrowDownRight/></a><a className="secondary" href="#journey">My Journey <ArrowRight/></a></div></div><figure className="portrait"><img src="/aditya-singaraju.jpg" alt="Portrait of Dr. Aditya Singaraju" fetchPriority="high"/><figcaption>Architect · Educator · Researcher</figcaption></figure></section>
  ...
  {siteConfig.electionMode?<section className="newChapter"><p>A new chapter for IIA Telangana<br/><strong>Election · {siteConfig.electionDates}</strong></p><h2>Together, let us build a Chapter that is <em>inclusive, inspiring and accountable.</em></h2><a href="#vision">Support My Candidature <ArrowRight/></a></section>:null}

  </main>{siteConfig.electionMode?<a className="mobileElectionCta" href="#vision"><span><b>{countdown.value}</b> {countdown.label}</span><strong>Support My Candidature <ArrowRight/></strong></a>:null}<footer><a className="brand inverse" href="#top"><span>AS</span><b>Dr. Aditya<br/>Singaraju</b></a><p>{profile.roles}</p><p>© {new Date().getFullYear()} Dr. Aditya Singaraju</p></footer></>}
